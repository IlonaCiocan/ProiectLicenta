...
namespace Mechanisms
{
    public class Database
    {
        ...
        public static extern int export_argon2_blake2(byte[] password,
                                                    uint passwordLen,
                                                    [In, Out] byte[] hash,
                                                    uint hash_len,
                                                    byte[] salt,
        ...
        public byte[] HashPsw(string passwordStr)
        {
            ...
            int asd = export_argon2_blake2(passwordByteArr, (uint)passwordByteArr.Length, hash1, hash_len, saltByteArr, salt_len, t_cost, m_cost, parallelism);
        ...
        public void ResetPwd(string username, byte[] hashdefault, string who)
        {
            using (SQLiteConnection _dbConnection = new SQLiteConnection(connectionstring))
            using (SQLiteCommand command = _dbConnection.CreateCommand())
            {
                ...
                command.ExecuteNonQuery();
                _dbConnection.Close();
                WriteToLogFile("Resetare_parola", username, "reusit", who);

            }
        }
        public void ModifyDepartament(string username, string departament,string who)
        {
            using (SQLiteConnection _dbConnection = new SQLiteConnection(connectionstring))
            using (SQLiteCommand command = _dbConnection.CreateCommand())
            {
                command.CommandText = "update Users set departament=@departament where name=@username";
        ...
        public void ModifyRoleDep(string username,string departament,string isadmin, string who)
        {
            using (SQLiteConnection _dbConnection = new SQLiteConnection(connectionstring))
            using (SQLiteCommand command = _dbConnection.CreateCommand())
            {
                command.CommandText = "update Users set  isadmin=@isadmin,departament=@departament where name=@username";
